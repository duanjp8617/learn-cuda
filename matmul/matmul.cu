...
__global__ void matmul_1d_tiling(const float *A, const float *B, float *C) {
  ...
  for (int k = 0; k < CEIL_DIV(K, BK); k++) {
    // Move A_tile pointer towards the row (K-dimention) until the end
    A_tile = A_tile + k * BK;
    B_tile = B_tile + k * BK * N;
    ...
    for (int tid = threadIdx.x; tid < BM * BK; tid += BM * BN) {
      ...
      A_tile_cache[row][col] =
          row < M_remaining && col < K_remaining ? A_tile[row * K + col] : 0;
    }
    ...
    for (int tid = threadIdx.x; tid < BK * BN; tid += BM * BN) {
      ...
      B_tile_cache[row][col] =
          row < K_remaining && col < N_remaining ? B_tile[row * N + col] : 0;
    }
    ...
    for (int k = 0; k < BK; k++) {
      accum +=
          A_tile_cache[threadIdx.x / BN][k] * B_tile_cache[k][threadIdx.x % BN];
    }
    row < M_remaining &&col < N_remaining ? C[row * N + col] = accum : 0;
  }
}