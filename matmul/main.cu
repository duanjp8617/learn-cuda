...
template <int M, int N, int K, int BM, int BN, int BK>
void launch_matmul_iterator(const float *device_a, const float *device_b,
                            float *device_c) {
  const dim3 block(BM * BN);
  const dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
  matmul_1d_tiling_iterator<M, N, K, BM, BN, BK><<<grid, block>>>(device_a, device_b,
                                                                    device_c);
}

template <int M, int N, int K>
void launch_matmul_2d(const float *device_a, const float *device_b, float *device_c) {
  constexpr int BM = 128;
  constexpr int BN = 128;
  const dim3 block(256);
  const dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
  matmul_2d_tiling<<<grid, block>>>(M, N, K, device_a, device_b, device_c);
}

template <int M, int N, int K>
void launch_matmul_2d_iterator(const float *device_a, const float *device_b,
                               float *device_c) {
  constexpr int BM = 128;
  constexpr int BN = 128;
  const dim3 block(256);
  const dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
  matmul_2d_tiling_iterator<<<grid, block>>>(M, N, K, device_a, device_b, device_c);
}
...
bool verify_result(const std::vector<float> &a, const std::vector<float> &b,
                   const std::vector<float> &c, const char *kernel_name) {
  ...
  std::printf("%s correctness: %s (max absolute error %.8f)\n", kernel_name,
...
bool run_correctness_test() {
  ...
  return verify_result<M, N, K>(host_a, host_b, host_c,
                                "matmul_1d_tiling (manual)");
}

template <int M, int N, int K, int BM, int BN, int BK>
bool run_correctness_test_iterator() {
  std::vector<float> host_a(static_cast<size_t>(M) * K);
  std::vector<float> host_b(static_cast<size_t>(K) * N);
  std::vector<float> host_c(static_cast<size_t>(M) * N);
  initialize_inputs<M, N, K>(host_a, host_b);
  ...
  launch_matmul_iterator<M, N, K, BM, BN, BK>(device_a, device_b, device_c);
  CHECK_CUDA(cudaGetLastError());
  CHECK_CUDA(cudaDeviceSynchronize());
  CHECK_CUDA(cudaMemcpy(host_c.data(), device_c, host_c.size() * sizeof(float), cudaMemcpyDeviceToHost));
  ...
  CHECK_CUDA(cudaFree(device_c));
  return verify_result<M, N, K>(host_a, host_b, host_c,
                                "matmul_1d_tiling_iterator");
}

template <int M, int N, int K>
bool run_correctness_test_2d() {
  std::vector<float> host_a(static_cast<size_t>(M) * K);
  std::vector<float> host_b(static_cast<size_t>(K) * N);
  std::vector<float> host_c(static_cast<size_t>(M) * N);
  initialize_inputs<M, N, K>(host_a, host_b);
  ...
  launch_matmul_2d<M, N, K>(device_a, device_b, device_c);
  CHECK_CUDA(cudaGetLastError());
  CHECK_CUDA(cudaDeviceSynchronize());
  CHECK_CUDA(cudaMemcpy(host_c.data(), device_c, host_c.size() * sizeof(float), cudaMemcpyDeviceToHost));
  ...
  CHECK_CUDA(cudaFree(device_c));
  return verify_result<M, N, K>(host_a, host_b, host_c,
                                "matmul_2d_tiling (manual)");
}

template <int M, int N, int K>
bool run_correctness_test_2d_iterator() {
  std::vector<float> host_a(static_cast<size_t>(M) * K);
  std::vector<float> host_b(static_cast<size_t>(K) * N);
  std::vector<float> host_c(static_cast<size_t>(M) * N);
  initialize_inputs<M, N, K>(host_a, host_b);
  ...
  launch_matmul_2d_iterator<M, N, K>(device_a, device_b, device_c);
  CHECK_CUDA(cudaGetLastError());
  CHECK_CUDA(cudaDeviceSynchronize());
  CHECK_CUDA(cudaMemcpy(host_c.data(), device_c, host_c.size() * sizeof(float), cudaMemcpyDeviceToHost));
...
BenchmarkResult run_benchmark() {
  ...
  std::printf("matmul_1d_tiling (manual)\n");
...
template <int M, int N, int K, int BM, int BN, int BK>
BenchmarkResult run_benchmark_iterator() {
  constexpr int warmup_iterations = 5;
  constexpr int timed_iterations = 50;
  std::vector<float> host_a(static_cast<size_t>(M) * K);
  std::vector<float> host_b(static_cast<size_t>(K) * N);
  ...
  const double average_ms = elapsed_ms / timed_iterations;
  const double gflops = 2.0 * M * K * static_cast<double>(N) / average_ms * 1.0e-6;
  std::printf("matmul_1d_tiling_iterator\n");
  std::printf("  C(%d,%d) = A(%d,%d) @ B(%d,%d), block %dx%d, BK %d\n", M, N, M, K, K, N,
              BM, BN, BK);
  std::printf("  Average time: %.3f ms (%d iterations)\n", average_ms, timed_iterations);
  std::printf("  Throughput: %.2f GFLOP/s\n", gflops);
...
template <int M, int N, int K>
BenchmarkResult run_benchmark_2d() {
  constexpr int warmup_iterations = 5;
  constexpr int timed_iterations = 50;
  std::vector<float> host_a(static_cast<size_t>(M) * K);
  std::vector<float> host_b(static_cast<size_t>(K) * N);
  ...
  const double average_ms = elapsed_ms / timed_iterations;
  const double gflops = 2.0 * M * K * static_cast<double>(N) / average_ms * 1.0e-6;
  std::printf("matmul_2d_tiling (manual)\n");
  std::printf("  C(%d,%d) = A(%d,%d) @ B(%d,%d), block 128x128, BK 32, threads 256\n",
              M, N, M, K, K, N);
  std::printf("  Average time: %.3f ms (%d iterations)\n", average_ms, timed_iterations);
  std::printf("  Throughput: %.2f GFLOP/s\n", gflops);
...
template <int M, int N, int K>
BenchmarkResult run_benchmark_2d_iterator() {
  constexpr int warmup_iterations = 5;
  constexpr int timed_iterations = 50;
  std::vector<float> host_a(static_cast<size_t>(M) * K);
  std::vector<float> host_b(static_cast<size_t>(K) * N);
  ...
  const double average_ms = elapsed_ms / timed_iterations;
  const double gflops = 2.0 * M * K * static_cast<double>(N) / average_ms * 1.0e-6;
  std::printf("matmul_2d_tiling_iterator\n");
  std::printf("  C(%d,%d) = A(%d,%d) @ B(%d,%d), block 128x128, BK 32, threads 256\n",
              M, N, M, K, K, N);
  std::printf("  Average time: %.3f ms (%d iterations)\n", average_ms, timed_iterations);
  std::printf("  Throughput: %.2f GFLOP/s\n", gflops);
...
void print_iterator_comparison(const char *tiling_name,
                               BenchmarkResult manual,
                               BenchmarkResult iterator) {
  const double speedup = manual.average_ms / iterator.average_ms;
  std::printf("%s iterator vs manual: ", tiling_name);
  if (speedup >= 1.0) {
    std::printf("iterator speedup %.2fx\n", speedup);
  } else {
    std::printf("iterator slowdown %.2fx\n", 1.0 / speedup);
  }
}
...
int main() {
  ...
  if (!run_correctness_test_iterator<37, 29, 53, BM, BN, BK>()) {
    return EXIT_FAILURE;
  }
  if (!run_correctness_test_2d<37, 29, 53>()) {
    return EXIT_FAILURE;
  }
  if (!run_correctness_test_2d_iterator<37, 29, 53>()) {
    return EXIT_FAILURE;
  }
  if (!run_correctness_test_2d<129, 130, 33>()) {
    return EXIT_FAILURE;
  }
  if (!run_correctness_test_2d_iterator<129, 130, 33>()) {
    return EXIT_FAILURE;
  }

//   const BenchmarkResult manual_1d = run_benchmark<1024, 1024, 1024, BM, BN, BK>();
//   const BenchmarkResult iterator_1d =
//       run_benchmark_iterator<1024, 1024, 1024, BM, BN, BK>();
//   print_iterator_comparison("1D tiling", manual_1d, iterator_1d);

  const BenchmarkResult manual_2d = run_benchmark_2d<1024, 1024, 1024>();
  // const BenchmarkResult iterator_2d = run_benchmark_2d_iterator<1024, 1024, 1024>();
  // print_iterator_comparison("2D tiling", manual_2d, iterator_2d);
  return EXIT_SUCCESS;
}
